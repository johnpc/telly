...
class AssignEpgSessionTest {
    ...
    fun `picking an id persists the override and closes the tool`() {
        runTest {
            ...
            assertEquals("sports.fixture", updates.single().overrides.epgOverride)
        ...
        runTest {
            val channel =
                testChannel(1, 1, "News One").let {
                    it.copy(overrides = it.overrides.copy(epgOverride = "sports.fixture"))
                }
            ...
            assertNull(updates.single().overrides.epgOverride)
