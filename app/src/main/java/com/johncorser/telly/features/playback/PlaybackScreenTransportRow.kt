...
import com.johncorser.telly.features.catchup.CatchupKeyPolicy
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            transportButtons.forEachIndexed { index, (icon, feature) ->
                TellyScreenIconCircle(
                    icon = icon,
                    onClick = { transportAction(index, feature, onFeature, onSeek) },
...
private fun transportAction(
    ...
    onSeek != null && index == REWIND_INDEX -> onSeek(-CatchupKeyPolicy.SKIP_BACK_MS)
    onSeek != null && index == FORWARD_INDEX -> onSeek(CatchupKeyPolicy.SKIP_FORWARD_MS)
