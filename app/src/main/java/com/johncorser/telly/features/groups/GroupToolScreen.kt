...
fun GroupToolScreen(session: GroupToolSession) {
    ...
    SettingsScreenSheet(title = ui.title) {
        Box(Modifier.testTag("group-tool")) {
            when {
                ui.pin -> GroupToolScreenPin(onSubmit = session::submitPin)
                ui.textInitial != null ->
...
/** The blocking editor's entry gate: the shared PIN wheel in the sheet. */
@Composable
private fun GroupToolScreenPin(onSubmit: (String) -> Unit) {
    Column(
        Modifier
            .fillMaxWidth()
            .padding(24.dp),
