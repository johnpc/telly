...
internal fun SettingsViewModel.runBlockedOrSearchAction(rowId: String) {
    when (rowId) {
        ...
        BlockRowIds.SEARCH_CLEAR_HISTORY -> showOverlay(SettingsOverlay.ConfirmClearHistory)
...
internal fun SettingsViewModel.unblockChannel(rowId: String) {
    val channelId = rowId.removePrefix(BlockRowIds.CHANNEL_PREFIX).toLongOrNull() ?: return
