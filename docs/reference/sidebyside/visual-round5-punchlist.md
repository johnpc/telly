...
| Focus re-lands on originating row after BACK | **ANSWERED (round6): reference RESTORES the originating row** — sheet opened from the 5th visible row (ch 5, News One 2, [0,702][1920,780]); after BACK that same row is focused=true (`ref-round6/04-row5-*.xml`; the row also keeps a white outline while the sheet is open) | **FIXED** — `GuideFocusMemory` snapshots the grid focus (channel id, cell, anchor, scrollX, firstRow) when the sheet opens and re-asserts it, consumed once, on the first transition back to `GuideLayer.Grid` (landed on main as e66bbed; round6 §B confirms restore-to-origin is the reference behavior) | MATCH (reference-confirmed; on-device eyeball still due) |
| BACK from Channel options returns to the "Channel options" row | **ANSWERED (round6): N/A — the reference pane REPLACES the sheet** (cross-fade in place, sheet fades out on push); one BACK from Channel options lands directly on the guide grid with the originating row focused, never back on the sheet (`ref-round6/03-after-back-from-channel-options.png/.xml`, `03-channel-options-push-pop.mp4`) | **FIXED — telly adopted the replace semantics (2026-09-14)**: Channel options BACK now lands directly on the grid (guide: `GuideMenuController.backOf` → `Grid`, `GuideFocusMemory` restores the originating row) / the panel (playback: `PlaybackOverlay.ChannelOptions.back = Panel`); `PlayerMenuFocus` no longer remembers the Channel-options row (dead path — the sheet is never returned to). The sheet-restore behavior stays for the OTHER pushed layers (description/paywall/coming-soon), uncaptured in round 6, per the routing table's one-level BACK | MATCH (semantics; pane cross-fade motion itself not yet animated — sheet cut + pane appears via layer switch) |
...
- Guide sheet BACK focus restore (originating row / Channel-options row) —
  ...
  **FIXED (2026-09-14):** both — `GuideFocusMemory` restore (e66bbed) and
  the Channel-options replace semantics + sheet entrance/scrim motion (this
  change); see the updated table rows and `motion-punchlist.md` §P2.
